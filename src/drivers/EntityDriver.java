...
public class EntityDriver implements DriverInterface {
    ...
    /**
     * This method will be the main method of this class. It will test each class in the entities package.
     */
    @Override
    public boolean test() {
    ...
    /**
     * This will test the all instances of Enemies and return the result the tests. Cannot test Enemy itself since it is an abstract class.
    ...
    private boolean testEnemy() {
        ...
        // create a Skeleton at a point that is above the ground so they are in air. 4 tiles on x and y is in the air.
        ...
        Player testPlayer = new Player(skelX + 1 * Game.TILES_SIZE, skelY + 2 * Game.TILES_SIZE, skelWidth, skelHeight, null);
        ...
        // an update buffer to make sure that the loop isn't infinite. This should be a high number because updates happen slowly.
        ...
        testPlayer.update(0);
        System.out.println((testSkel.getHitbox().y + testSkel.getHitbox().height) + "  " + (testPlayer.getHitbox().y + testPlayer.getHitbox().height));
        
        return allSuccess;
    }
    
    /**
     * This is used to style the messages that are printed if any tests failed in the testEnemy Method
