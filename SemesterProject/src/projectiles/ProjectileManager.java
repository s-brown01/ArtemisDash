 ...
 * @date Mar 28, 2024
 * @description
 * This class will be a projectile manager/factory for the Playing GameState. This allows all of the moving/collision logic to be stored outside of the Playing state
 */
...
//        final BufferedImage arrowSprite = LoadSave.getSpriteSheet("TEST");
        // the first dimension is how many types of arrows. The second is the maximum number of sprites
        ...
        for (int i = 0; i < arrowImgs.length; i++) {
            for (int j = 0; j < arrowImgs[i].length; i++) {
                // grab all of the sprites from that row. width and height should be determined on the arrow size
//                arrowImgs[i][j] = arrowSprite.getSubimage(1, 1, 1, 11);
    ...
    /**
     * This method updates each projectile and checks their end-case (when they should de-spawn)
     */
    public void update() {
